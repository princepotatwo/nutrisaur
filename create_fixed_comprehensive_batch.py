...
def create_fixed_comprehensive_batch():
    print("Creating comprehensive test batch with UNIQUE emails for all classifications...")
        ...
        for classification, weights in male_cases.items():
            for i, weight in enumerate(weights):
                ...
                test_data.append([
                    ...
                    f"test{record_counter}@example.com",  # UNIQUE EMAIL
                    "password123",
                    "CITY OF BALANGA",
                    "Bagumbayan",
                    "Male",
    ...
    # Write to CSV
    with open('fixed_comprehensive_test.csv', 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file, quoting=csv.QUOTE_ALL)
        writer.writerow(header)
        writer.writerows(test_data)
    
    print(f"Created fixed_comprehensive_test.csv with {len(test_data)} test cases")
    print("✅ All emails are UNIQUE (test1@example.com, test2@example.com, etc.)")
    ...
    for i in range(min(15, len(test_data))):
        print(f"  {test_data[i][0]}: {test_data[i][8]}kg, {test_data[i][5]}, Email: {test_data[i][1]}")

if __name__ == "__main__":
    create_fixed_comprehensive_batch()
