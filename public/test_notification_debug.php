...
try {
    ...
    echo "\n2. Testing getActiveFCMTokens method...\n";
    
    // 4. Test the getActiveFCMTokens method
    $activeTokens = $db->getActiveFCMTokens();
    echo "   - getActiveFCMTokens returned: " . count($activeTokens) . " tokens\n";
    
    if (count($activeTokens) > 0) {
        echo "   - Sample active tokens:\n";
        foreach (array_slice($activeTokens, 0, 3) as $token) {
            echo "     * " . $token['user_email'] . " (" . $token['user_barangay'] . ") -> " . substr($token['fcm_token'], 0, 20) . "...\n";
        }
    }
    
    echo "\n3. Testing notification API...\n";
    
    // 5. Test the notification API endpoint
    $testData = [
    ...
    echo "\n4. Testing FCM function directly...\n";
    
    // 6. Test FCM function directly
    if (count($activeTokens) > 0) {
        $testToken = $activeTokens[0]['fcm_token'];
