<?php
// Test version of Google OAuth API with better error handling
...
try {
    ...
    if (empty($idToken) || empty($email)) {
        throw new Exception('Missing required parameters - id_token: ' . (empty($idToken) ? 'missing' : 'present') . ', email: ' . (empty($email) ? 'missing' : 'present'));
    }
    ...
    if (!$response) {
        throw new Exception('Failed to verify token with Google - no response');
    }
    
    $tokenInfo = json_decode($response, true);
    
    if (!$tokenInfo) {
        throw new Exception('Invalid token response from Google: ' . $response);
    }
    
    // Verify the audience (client ID)
    if ($tokenInfo['aud'] !== $webClientId && $tokenInfo['aud'] !== $androidClientId) {
        throw new Exception('Invalid client ID - expected: ' . $webClientId . ' or ' . $androidClientId . ', got: ' . $tokenInfo['aud']);
    }
    
    // Verify the email matches
    if ($tokenInfo['email'] !== $email) {
        throw new Exception('Email mismatch - expected: ' . $email . ', got: ' . $tokenInfo['email']);
    }
